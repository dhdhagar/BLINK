...
def analyzeClusters(
    ...
    logger.info(
        f"{n_predicted} mentions assigned to {len(clusters)} clusters; {debug_no_pred} singelton clusters"
    )

    if not no_drop_seen:
        nmi = normalized_mutual_info_score(
            gold_cluster_labels, predicted_cluster_labels
        )
    ...
    for mode in idx_subsets:
        nmi = normalized_mutual_info_score(
            gold_cluster_labels[idx_subsets[mode]],
            predicted_cluster_labels[idx_subsets[mode]],
        )
        ...
        result = (nmi + rand_index) / 2
        logger.info(
            f"{mode.upper()}: NMI={nmi}, rand_index={rand_index} => average={result}"
        )
...
def main(params):
    ...
    mention_gold_cui_idxs = list(
        map(lambda x: x["label_idxs"][n_labels - 1], mention_data)
    )
    ...
    if params["drop_all_entities"]:
        ...
        n_mentions_wo_gold_ents = sum(
            [1 if x in set_dropped_ent_idxs else 0 for x in mention_gold_cui_idxs]
        )
        ...
        if use_types:
            dict_idxs_by_type = data_process.get_idxs_by_type(
                list(compress(dictionary, keep_mask))
            )
        ...
        if use_types:
            for men_type in men_idxs_by_type:
                ...
                q_men_embeds = men_embeds[
                    men_idxs_by_type[men_type]
                ]  # np.array(list(map(lambda x: men_embeds[x], men_idxs_by_type[men_type])))
                ...
                for i, men_idx in enumerate(men_idxs_by_type[men_type]):
                    ...
                    q_scores = torch.flatten(
                        torch.mm(
                            torch.tensor(q_men_embeds[i : i + 1]).cuda(), q_nn_embeds.T
                        )
                    ).cpu()
                    c, data = new_to_old_dict_mapping[
                        q_nn_idxs[torch.argmax(q_scores)]
                    ], torch.max(q_scores)
            ...
            for men_idx, men_embed in enumerate(men_embeds):
                ...
                q_scores = torch.flatten(
                    torch.mm(
                        torch.tensor(np.expand_dims(men_embed, axis=0)).cuda(),
                        q_nn_embeds.T,
                    )
                ).cpu()
                c, data = new_to_old_dict_mapping[
        ...
        for k in joint_graphs:
            if params["drop_all_entities"]:
                ...
                for ki in range(len(joint_graphs[k]["rows"])):
                    if (
                        joint_graphs[k]["cols"][ki] < n_entities
                        or joint_graphs[k]["rows"][ki] < n_entities
                    ):
                        continue
                    # Remove mentions whose gold entity was seen during training
                    if len(seen_mention_idxs) > 0 and not params["no_drop_seen"]:
                        if (
                            joint_graphs[k]["rows"][ki] - n_entities
                        ) in seen_mention_idxs or (
                            joint_graphs[k]["cols"][ki] - n_entities
                        ) in seen_mention_idxs:
                            continue
                ...
                (
                    ...
                    joint_graphs[k]["cols"],
                    joint_graphs[k]["data"],
                ) = list(map(np.array, (_f_row, _f_col, _f_data)))
            if (exact_knn is None and k > 0 and k <= knn) or (
                ...
                for thresh in thresholds:
                    ...
                    partitioned_graph, clusters = partition_graph(
                        ...
                        exclude=set_dropped_ent_idxs,
                        threshold=thresh,
                        without_entities=params["drop_all_entities"],
                    )
                    # Analyze cluster against gold clusters
                    result = analyzeClusters(
                        clusters,
                        mention_gold_cui_idxs,
                        n_entities,
                        n_mentions,
                        logger,
                        unseen_mention_idxs_map,
                        no_drop_seen=params["no_drop_seen"],
                    )
