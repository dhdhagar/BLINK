    ...
    for step, batch in enumerate(iter_):
        ...
        with torch.no_grad():
            ...
            for i, m_embed in enumerate(mention_embeddings):
                ...
                _, knn_dict_idxs = valid_dict_index.search(np.expand_dims(m_embed, axis=0).astype('float32'), knn)
    ...
    for epoch_idx in trange(int(num_train_epochs), desc="Epoch"):
        ...
        for step, batch in enumerate(iter_):
            ...
            for i, m_embed in enumerate(mention_embeddings):
                ...
                _, knn_dict_idxs = train_dict_index.search(np.expand_dims(m_embed, axis=0).astype('float32'), (knn + int(n_gold[i]) - 1)) # Fetch NNs to ensure one entity per row
                knn_dict_idxs = knn_dict_idxs.astype(np.int64).flatten()
                if use_types:
                    # Map type-specific indices to the entire dictionary
