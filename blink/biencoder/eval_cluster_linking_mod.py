...
def partition_graph(graph, n_entities, directed, return_clusters=False):
    ...
    rows, cols, data = cluster_linking_partition(
        graph["rows"], graph["cols"], graph["data"], n_entities, directed
    )
    # Construct the partitioned graph
    partitioned_graph = coo_matrix((data, (rows, cols)), shape=graph["shape"])
...
def main(params):
    ...
    if not params["only_recall"] and os.path.isfile(graph_path):
        ...
        if use_types:
            ...
            # Verifiy embeddings
            og_embeds = torch.load(
                "models/trained/zeshel_og/eval/data_og/cand_encodes.t7"
            )
            world_to_type = {
                12: "forgotten_realms",
                13: "lego",
        ...
        for men_query_idx, men_embed in enumerate(
            tqdm(men_embeds, total=len(men_embeds), desc="Fetching k-NN")
        ):
            ...
            gold_idxs = mention_data[men_query_idx]["label_idxs"][
                : mention_data[men_query_idx]["n_labels"]
            ]
            dict_cand_idx, dict_cand_score, recall_idx = get_query_nn(
                ...
                dict_index,
                men_embed,
                searchK=params["recall_k"],
                gold_idxs=gold_idxs,
                type_idx_mapping=dict_type_idx_mapping,
            )
            ...
            if not params["only_recall"]:
                ...
                men_cand_idxs, men_cand_scores = get_query_nn(
                    ...
                    men_index,
                    men_embed,
                    type_idx_mapping=men_type_idx_mapping,
                )
