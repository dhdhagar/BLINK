...
def score_in_batches(
    ...
    if n_inputs > 1:
        ...
        dataloader = DataLoader(cross_inputs, sampler=sampler, batch_size=batch_size)
        _iter = (
            dataloader
            if silent
            else tqdm(dataloader, desc=f"Scoring in batches (size={batch_size})")
        )
    for batch in _iter:
        ...
        if batch_shape[0] < cross_reranker.n_gpu:
            ...
            repeat_shape = [
                math.ceil(cross_reranker.n_gpu / float(batch_shape[0])),
                *[1] * (len(batch_shape) - 1),
            ]
            batch = batch.repeat(repeat_shape)[: cross_reranker.n_gpu]
        batch_scores = cross_reranker.score_candidate(
            batch.cuda(), max_context_length, is_context_encoder=is_context_encoder
        )
        if reshaped:
            reshaped = False
...
def evaluate(
    ...
    for men_idx in tqdm(
        range(len(valid_processed_data)),
        total=len(valid_processed_data),
    ...
    for k in joint_graphs:
        for mode in ["directed", "undirected"]:
            ...
            result = eval_cluster_linking.analyzeClusters(
                clusters, entity_dictionary, valid_processed_data, k
            )
...
def get_biencoder_nns(
    ...
    if not params["only_evaluate"]:
        # Train set
        biencoder_train_idxs_pkl_path = os.path.join(
            biencoder_indices_path, "biencoder_train_idxs.pickle"
        )
        if os.path.isfile(biencoder_train_idxs_pkl_path):
            ...
            if use_types:
                ...
                dict_embeddings, dict_index = data_process.embed_and_index(
                    bi_reranker,
                    entity_dict_vecs,
                    encoder_type="candidate",
                    n_gpu=n_gpu,
            ...
            logger.info(
                "Biencoder: Finding nearest mentions and entities for each mention..."
            )
            bi_dict_nns = np.zeros(
                (len(train_men_embeddings), k_dict_nns), dtype=int
            )  # negatives
            bi_men_nns = -1 * np.ones(
                (len(train_men_embeddings), k_men_nns), dtype=int
            )  # negatives
            bi_men_gold_nns = -1 * np.ones(
                (len(train_men_embeddings), k_men_nns), dtype=int
            )  # positives
            if not use_types:
                _, bi_dict_nns_np = dict_index.search(
                    train_men_embeddings, k_dict_nns + 1
                )
                _, bi_men_nns_np = train_men_index.search(
                    train_men_embeddings, len(train_men_embeddings)
                )
                for i in range(len(bi_dict_nns_np)):
                    gold_idx = train_processed_data[i]["label_idxs"][0]
                    bi_dict_nns[i] = bi_dict_nns_np[i][bi_dict_nns_np[i] != gold_idx][
                        :k_dict_nns
                    ]
                    men_neg_mask = ~np.isin(
                        bi_men_nns_np[i], train_gold_clusters[gold_idx]
                    )
                    neg_nns = bi_men_nns_np[i][men_neg_mask]
                    if within_doc:
                        neg_nns, _ = filter_by_context_doc_id(
                            neg_nns,
                            train_context_doc_ids[i],
                            train_context_doc_ids,
                            return_numpy=True,
                        )
                    bi_men_nns[i][: len(neg_nns[:k_men_nns])] = neg_nns[:k_men_nns]
                    men_gold_mask = (
                        np.isin(bi_men_nns_np[i], train_gold_clusters[gold_idx])
                    ) & (bi_men_nns_np[i] != i)
                    gold_nns = bi_men_nns_np[i][men_gold_mask]
                    if within_doc:
                        gold_nns, _ = filter_by_context_doc_id(
                            gold_nns,
                            train_context_doc_ids[i],
                            train_context_doc_ids,
                            return_numpy=True,
                    ...
                    bi_men_gold_nns[i][: len(gold_nns[:k_men_nns])] = gold_nns[
                        :k_men_nns
                    ]
            else:
                for entity_type in train_men_indexes:
                    ...
                    _, dict_nns_by_type = dict_indexes[entity_type].search(
                        men_embeds_by_type, k_dict_nns + 1
                    )
                    _, men_nns_by_type = train_men_indexes[entity_type].search(
                        men_embeds_by_type, len(men_embeds_by_type)
                    )
                    ...
                    for i, idx in enumerate(men_idxs_by_type[entity_type]):
                        ...
                        bi_dict_nns[idx] = dict_nns_idxs[i][
                            dict_nns_idxs[i] != gold_idx
                        ][:k_dict_nns]
                        men_neg_mask = ~np.isin(
                            men_nns_idxs[i], train_gold_clusters[gold_idx]
                        )
                        neg_nns = men_nns_idxs[i][men_neg_mask]
                        if within_doc:
                            neg_nns, _ = filter_by_context_doc_id(
                                neg_nns,
                                train_context_doc_ids[idx],
                                train_context_doc_ids,
                                return_numpy=True,
                            )
                        bi_men_nns[idx][: len(neg_nns[:k_men_nns])] = neg_nns[
                            :k_men_nns
                        ]
                        men_gold_mask = (
                            np.isin(men_nns_idxs[i], train_gold_clusters[gold_idx])
                        ) & (men_nns_idxs[i] != idx)
                        ...
                        # If len(gold_nns) < k_men_nns then set bi_men_gold_nns[i] = [...gold_nns, -1, -1, ...]
                        bi_men_gold_nns[idx][: len(gold_nns[:k_men_nns])] = gold_nns[
                            :k_men_nns
                        ]
            ...
            with open(biencoder_train_idxs_pkl_path, "wb") as write_handle:
                pickle.dump(
                    biencoder_train_idxs, write_handle, protocol=pickle.HIGHEST_PROTOCOL
                )
            logger.info("Biencoder: Saved")

    # Dev set
    biencoder_valid_idxs_pkl_path = os.path.join(
        biencoder_indices_path, "biencoder_valid_idxs.pickle"
    )
    if os.path.isfile(biencoder_valid_idxs_pkl_path):
        ...
        if use_types:
            ...
            (
                valid_men_embeddings,
                valid_men_indexes,
                valid_men_idxs_by_type,
            ) = data_process.embed_and_index(
                bi_reranker,
                valid_men_vecs,
                encoder_type="context",
                n_gpu=n_gpu,
                corpus=valid_processed_data,
                force_exact_search=True,
                batch_size=params["embed_batch_size"],
            )
        else:
            ...
            valid_men_embeddings, valid_men_index = data_process.embed_and_index(
                bi_reranker,
                valid_men_vecs,
                encoder_type="context",
                n_gpu=n_gpu,
        ...
        if not use_types:
            ...
            _, bi_men_nns_np = valid_men_index.search(
                valid_men_embeddings, n_mens_to_fetch
            )
            for i in range(len(bi_men_nns_np)):
                bi_dict_nns[i] = bi_dict_nns_np[i]
                men_nns = bi_men_nns_np[i][bi_men_nns_np[i] != i]
                if within_doc:
                    men_nns, _ = filter_by_context_doc_id(
                        men_nns,
                        valid_context_doc_ids[i],
            ...
            for entity_type in valid_men_indexes:
                men_embeds_by_type = valid_men_embeddings[
                    valid_men_idxs_by_type[entity_type]
                ]
                _, dict_nns_by_type = dict_indexes[entity_type].search(
                    men_embeds_by_type, k_dict_nns
                )
                _, men_nns_by_type = valid_men_indexes[entity_type].search(
                    men_embeds_by_type, len(men_embeds_by_type)
                )
                dict_nns_idxs = np.array(
                    list(
                ...
                men_nns_idxs = np.array(
                    list(
                        map(
                            lambda x: valid_men_idxs_by_type[entity_type][x],
                            men_nns_by_type,
                        )
                    )
                )
        ...
        with open(biencoder_valid_idxs_pkl_path, "wb") as write_handle:
            pickle.dump(
                biencoder_valid_idxs, write_handle, protocol=pickle.HIGHEST_PROTOCOL
            )
        logger.info("Biencoder: Saved")
...
def drop_entities_for_discovery_training(
    entity_dictionary, drop_set_fname, logger, ent_drop_prop=0.1
):
...
def load_training_data(
    ...
    if (
        not params["drop_entities"]
        and os.path.isfile(train_tensor_data_pkl_path)
        and os.path.isfile(train_processed_data_pkl_path)
    ):
        ...
        if params["drop_entities"]:
            ...
            entity_dictionary = drop_entities_for_discovery_training(
                entity_dictionary, params["drop_set"], logger
            )
...
def get_gold_arbo_links(
    ...
    with torch.no_grad():
        ...
        _iter = tqdm(
            range(len(train_men_vecs) if not debug else 200),
            desc="Computing gold arbos",
        )
        for mention_idx in _iter:
            ...
            if mention_idx not in gold_links:
                ...
                to_ent_input = concat_for_crossencoder(
                    train_men_vecs[cluster_mens],
                    entity_dict_vecs[cluster_ent].expand(
                        len(cluster_mens), 1, entity_dict_vecs.size(1)
                    ),
                    max_seq_length,
                )  # Shape: N x 1 x 2D
                try:
                    to_ent_data = score_in_batches(
                        cross_reranker,
                        max_context_length,
                        to_ent_input,
                        is_context_encoder=False,
                    ...
                    raise ValueError(
                        f"Probably a batch size error. The current cluster size was {len(cluster_mens)}"
                    )
                to_men_input = concat_for_crossencoder(
                    train_men_vecs[cluster_mens],
                    train_men_vecs[cluster_mens].expand(
                        len(cluster_mens), len(cluster_mens), train_men_vecs.size(1)
                    ),
                    max_seq_length,
                )  # Shape: N x N x 2D
                try:
                    to_men_data = score_in_batches(
                        cross_reranker,
                        max_context_length,
                        to_men_input,
                        is_context_encoder=True,
...
def construct_train_batch(
    ...
    for mention_idx in mention_idxs:
        ...
        pos_label_input = concat_for_crossencoder(
            train_men_vecs[mention_idx : mention_idx + 1], to_input, max_seq_length
        )  # Shape: 1 x 1 x 2D
        pos_score = score_in_batches(
            cross_reranker,
            max_context_length,
            pos_label_input.cuda(),
            is_context_encoder=is_context_encoder,
        )
        batch_positive_scores = (
            pos_score
            if batch_positive_scores is None
            else torch.cat((batch_positive_scores, pos_score), dim=0)
        )
    ...
    return (
        batch_positive_scores,
        batch_negative_ent_inputs.cuda(),
        batch_negative_men_inputs.cuda(),
    )


def build_cross_concat_input(
    biencoder_idxs, men_vecs, entity_dict_vecs, max_seq_length, k_biencoder
):
    men_men_inputs = []
    men_ent_inputs = []
    for mention_idx in tqdm(range(len(men_vecs))):
        ...
        for bi_men_idx in bi_men_idxs:
            bi_men_inputs = (
                men_vecs[bi_men_idx].unsqueeze(dim=0)
                if bi_men_inputs is None
                else torch.cat(
                    (bi_men_inputs, men_vecs[bi_men_idx].unsqueeze(dim=0)), dim=0
                )
            )
        ...
        for bi_ent_idx in bi_ent_idxs:
            bi_ent_inputs = (
                entity_dict_vecs[bi_ent_idx].unsqueeze(dim=0)
                if bi_ent_inputs is None
                else torch.cat(
                    (bi_ent_inputs, entity_dict_vecs[bi_ent_idx].unsqueeze(dim=0)),
                    dim=0,
                )
            )
        bi_ent_inputs = torch.unsqueeze(bi_ent_inputs, dim=0)
        # Concatenate for cross-encoder
        cross_men_inputs = concat_for_crossencoder(
            men_vecs[mention_idx : mention_idx + 1], bi_men_inputs, max_seq_length
        )  # Shape: 1 x knn x 2D
        cross_ent_inputs = concat_for_crossencoder(
            men_vecs[mention_idx : mention_idx + 1], bi_ent_inputs, max_seq_length
        )  # Shape: 1 x knn x 2D
...
def get_train_neg_cross_inputs(
    cross_reranker,
    max_context_length,
    ...
    with torch.no_grad():
        ...
        neg_men_topk_inputs = _helper(
            concat_inputs=train_men_concat_inputs,
            n_knn=n_knn_men_negs,
            is_context_encoder=True,
        )
        logger.info("Scoring mention-entity negative edges using cross-encoder...")
        neg_ent_topk_inputs = _helper(
            concat_inputs=train_ent_concat_inputs,
            n_knn=n_knn_ent_negs,
            is_context_encoder=False,
        )
...
def execute_training_step(
    ...
    min_negs = float("inf")
    dual_negs_mask = np.array(
        [True] * len(mention_idxs)
    )  # Mask to keep queries with both ent and men negs
    skipped = 0
    ...
    if min_negs != float("inf"):  # i.e. at least 1 query has dual negs
        ...
        data_parallel_batch_size_check = (
            negative_men_inputs.shape[0] * negative_men_inputs.shape[1] >= n_gpu
            and negative_ent_inputs.shape[0] * negative_ent_inputs.shape[1] >= n_gpu
        )
        if data_parallel_batch_size_check:
            loss_dual_negs = cross_reranker(
                positive_scores,
                negative_men_inputs,
                negative_ent_inputs,
                max_context_length,
    ...
    loss = (
        (loss_dual_negs * (len(mention_idxs) - skipped) + loss_ent_negs * skipped)
        / len(mention_idxs)
    ) / grad_acc_steps
    loss.backward()
    return loss.item()
...
def main(params):
    ...
    knn = params[
        "knn"
    ]  # Number of (positive+negatives) in each row of the training batch
    ...
    params["train_batch_size"] = params["train_batch_size"] // grad_acc_steps
    ...
    if params["only_evaluate"]:
        entity_dictionary = load_training_data(
            bi_tokenizer,
            max_context_length,
            max_cand_length,
            knn,
        ...
        # Load training data
        entity_dictionary, train_tensor_data, train_processed_data = load_training_data(
            bi_tokenizer,
            max_context_length,
            max_cand_length,
            knn,
    ...
    valid_tensor_data, valid_processed_data = load_validation_data(
        bi_tokenizer,
        max_context_length,
        max_cand_length,
        entity_dictionary,
    ...
    if params["only_evaluate"]:
        ...
        valid_men_concat_inputs, valid_ent_concat_inputs = build_cross_concat_input(
            biencoder_valid_idxs,
            valid_men_vecs,
            entity_dict_vecs,
            max_seq_length,
    ...
    biencoder_train_idxs, biencoder_valid_idxs = get_biencoder_nns(
        bi_reranker=bi_reranker,
        biencoder_indices_path=biencoder_indices_path,
        entity_dictionary=entity_dictionary,
        entity_dict_vecs=entity_dict_vecs,
    ...
    valid_men_concat_inputs, valid_ent_concat_inputs = build_cross_concat_input(
        biencoder_valid_idxs, valid_men_vecs, entity_dict_vecs, max_seq_length, bi_knn
    )
    logger.info("Done")
    # Compute and store the concatenated cross-encoder inputs for the negatives in training
    logger.info(
        "Computing the concatenated cross-encoder negative inputs for training..."
    )
    train_men_concat_inputs, train_ent_concat_inputs = build_cross_concat_input(
        biencoder_train_idxs, train_men_vecs, entity_dict_vecs, max_seq_length, bi_knn
    )
    logger.info("Done")
    ...
    train_sampler = (
        ...
        if params["shuffle"]
        else SequentialSampler(train_tensor_data)
    )
    ...
    best_score = {
        "directed": {"acc": -1.0, "epoch": 0, "k": 0},
        "undirected": {"acc": -1.0, "epoch": 0, "k": 0},
    }
    n_knn_negs = params[
        "knn_negs"
    ]  # Number of k-NN negatives in each row of the training batch (default: 8)
    n_knn_ent_negs, n_knn_men_negs = n_knn_negs // 2, n_knn_negs // 2

    for epoch_idx in range(params["num_train_epochs"]):
        logger.info(
            f"***** Starting EPOCH {epoch_idx}/{params['num_train_epochs']-1} *****"
        )
        torch.cuda.empty_cache()
        ...
        if checkpoint_data is None:
            ...
            logger.info(
                "Computing gold arborescence links for positive training labels"
            )
            gold_links = get_gold_arbo_links(
                cross_reranker,
                max_context_length,
                entity_dict_vecs,
                train_men_vecs,
            ...
            neg_men_topk_inputs, neg_ent_topk_inputs = get_train_neg_cross_inputs(
                cross_reranker,
                max_context_length,
                train_men_concat_inputs,
                train_ent_concat_inputs,
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            total_loss = execute_training_step(
                mention_idxs,
                bi_train_nn_count,
                n_knn_men_negs,
                batch_positive_scores,
            ...
            if params["save_interval"] != -1:
                ...
                if (
                    train_prop_done != 1
                    and train_prop_done
                    >= (save_intervals_done + 1) * params["save_interval"]
                ):
                    ...
                    utils.save_model(
                        cross_model, cross_tokenizer, epoch_output_folder_path
                    )
        ...
        utils.save_model(
            cross_model, cross_tokenizer, epoch_output_folder_path, scheduler, optimizer
        )
        logger.info(f"Model saved at {epoch_output_folder_path}")
        ...
        if (
            params["checkpoint_epoch_data"]
            and epoch_idx < params["num_train_epochs"] - 1
        ):
            if os.path.isfile(checkpoint_pkl_path):
